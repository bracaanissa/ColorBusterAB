...
/**
 * Represents the game board for the ColorBuster game. 
 * Manages tiles, including their placement and removal.
 */
...
public class Board {
    ...
    private int cols;
    private int rows;
    private int threshold; // Used to determine when to reinitialize the board or collapse columns.
    
    /**
     * Constructs a Board with specified dimensions and threshold.
     * 
     * @param rows Number of rows in the board.
     * @param cols Number of columns in the board.
     * @param threshold The minimum group size required to trigger tile removal.
     */
	...
	/**
     * Initializes the board with Tiles, ensuring at least one move is available.
     */
	...
	private void collapseColumn(int colToColapse) {
        ...
        // Adding new tiles to the top at random
    ...
    public String toString() {
        ...
        for (int row = rows - 1; row >= 0; row--) {
            for (int col = 0; col < cols; col++) {
                s.append(tiles[row][col] != null ? tiles[row][col].toString() : "[-----]");
            }
         s.append("\n");
        }
        return s.toString();
